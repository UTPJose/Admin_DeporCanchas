...
- **Reportes y Análisis (`/reportes`) también es solo-super-admin**, misma restricción que gestionar administradores. En `GET /api/reports` (`app/api/reports/route.ts`) se distingue por `type`: `dashboard`, `by-deport` y `revenue` (sin `startDate`/`endDate`, solo `period`) quedan abiertos a cualquier admin porque el `/dashboard` general también los consume; `revenue` **con** `startDate`+`endDate` (el análisis detallado real de `/reportes`) y `by-property`/`top-users`/`compare` exigen `requireSuperAdmin()` → 403 para admin regular. En el frontend, `app/(dashboard)/reportes/page.tsx` muestra "Acceso restringido" si `!user?.isSuper`, y el link "Reportes" del `Sidebar` (`components/layout/Sidebar.tsx`) se oculta del todo para admins regulares (mismo patrón que el tab "Administradores" en Configuración).
